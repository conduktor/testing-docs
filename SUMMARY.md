# Table of contents

* [Testing Documentation](README.md)
* [Learn Apache Kafka with Conduktor](learn-apache-kafka-with-conduktor.md)

## Getting Started

* [Sign up to Conduktor Testing](getting-started/sign-up-to-conduktor-testing.md)
* [Setup the Testing Agent](getting-started/install-the-testing-agent.md)
* [Connect to a Kafka Cluster](getting-started/connect-to-a-kafka-cluster.md)
* [Create a Test Scenario](getting-started/create-a-test-scenario.md)

## Features

* [Workspace](features/workspace.md)
* [Building Tests](features/building-tests/README.md)
  * [Test Suites](features/building-tests/test-suites.md)
  * [Test Scenarios](features/building-tests/test-scenarios.md)
  * [Tasks](features/building-tests/tasks/README.md)
    * [Producer Task](features/building-tests/tasks/producer-task.md)
    * [Consumer Task](features/building-tests/tasks/consumer-task.md)
    * [Set Variable Task](features/building-tests/tasks/set-variable-task.md)
    * [Task Ports](features/building-tests/tasks/task-ports.md)
    * [HTTP Task](features/building-tests/tasks/http-task.md)
  * [Test Checks](features/building-tests/test-checks/README.md)
    * [Accessing Kafka message data](features/building-tests/test-checks/accessing-kafka-message-data/README.md)
      * [Check the partition of a message produced to Kafka](features/building-tests/test-checks/accessing-kafka-message-data/check-the-partition-of-a-message-produced-to-kafka.md)
      * [Check the value inside a JSON message consumed from Kafka](features/building-tests/test-checks/accessing-kafka-message-data/check-the-value-inside-a-json-message-consumed-from-kafka.md)
    * [Accessing Http response data](features/building-tests/test-checks/accessing-http-response-data.md)
    * [Check Operators](features/building-tests/test-checks/check-operators.md)
    * [Check Results](features/building-tests/test-checks/check-results.md)
  * [Chaining Tasks](features/building-tests/chaining-tasks.md)
  * [Schema Registry](features/building-tests/schema-registry.md)
* [Environments](features/environments/README.md)
  * [Using environment variables](features/environments/using-environment-variables.md)
* [Custom Inputs](features/custom-inputs.md)
* [DSL](features/dsl.md)
* [Insights](features/insights.md)
* [CI/CD Automation](features/ci-cd-automation.md)

## miscellaneous

* [FAQ](miscellaneous/faq.md)
* [Data Security](miscellaneous/data-security.md)
* [Changelog](miscellaneous/changelog.md)

## Tutorials

* [Event-Driven Retail System](tutorials/event-driven-retail-system.md)
